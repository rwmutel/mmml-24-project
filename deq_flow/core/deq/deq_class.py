...
class DEQBase(nn.Module):
    def __init__(self, args):
        ...
        self.eval_f_thres = (
            args.eval_f_thres
            if args.eval_f_thres > 0
            else int(self.f_thres * args.eval_factor)
        )
    ...
    def _log_convergence(self, info, name="FORWARD", color="yellow"):
        ...
        rel_lowest, abs_lowest = (
            info["rel_lowest"].mean().item(),
            info["abs_lowest"].mean().item(),
        )
        nstep = info["nstep"]

        show_str = (
            f"{state} | {name} | rel: {rel_lowest}; abs: {abs_lowest}; nstep: {nstep}"
        )
    ...
    def forward(
        self, deq_func, z_init, log=False, sradius_mode=False, writer=None, **kwargs
    ):
        raise NotImplementedError
...
class DEQIndexing(DEQBase):
    def __init__(self, args):
        ...
        produce_grad = [
            backward_factory(grad_type=pg, tau=args.tau, sup_all=args.sup_all)
        ...
        if args.ift:
            ...
            produce_grad[-1] = backward_factory(
                ...
                b_solver_kwargs=dict(
                    threshold=args.b_thres, eps=args.b_eps, stop_mode=args.b_stop_mode
                ),
            )
    ...
    def forward(
        ...
        if self.training:
            _, trajectory, info = self._solve_fixed_point(
                deq_func, z_init, log=log, *kwargs
            )

            z_out = []
            for z_pred, produce_grad in zip(trajectory, self.produce_grad):
                z_out += produce_grad(
                    self, deq_func, z_pred
                )  # See lib/grad.py for the backward pass implementations
            ...
            z_star, _, info = self._solve_fixed_point(
                deq_func, z_init, log=log, f_thres=self.eval_f_thres
            )
...
class DEQSliced(DEQBase):
    def __init__(self, args):
        ...
        if args.n_losses > 1:
            self.indexing = [
                int(args.f_thres // args.n_losses) for _ in range(args.n_losses)
            ]
        else:
    ...
    def forward(
        self, deq_func, z_star, log=False, sradius_mode=False, writer=None, **kwargs
    ):
        if self.training:
            z_out = []
            for f_thres, produce_grad in zip(self.indexing, self.produce_grad):
                z_star, info = self._solve_fixed_point(
                    deq_func, z_star, f_thres=f_thres, log=log
                )
                z_out += produce_grad(
                    self, deq_func, z_star, writer=writer
                )  # See lib/grad.py for implementations
            ...
            # During inference, we directly solve for fixed point
            z_star, info = self._solve_fixed_point(
                deq_func, z_star, f_thres=self.eval_f_thres, log=log
            )
