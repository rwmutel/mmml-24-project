...
def fetch_optimizer(args, model):
    ...
    if args.schedule == "cosine":
        scheduler = optim.lr_scheduler.CosineAnnealingLR(
            optimizer, args.num_steps, eta_min=1e-6
        )
    else:
...
class Logger:
    ...
    def _print_training_status(self):
        ...
        training_str = "[Step {:6d}, lr {:.7f}]   ".format(
            self.total_steps + 1, self.scheduler.get_last_lr()[0]
        )
        metrics_str = ", ".join(
            [f"{name}:{val:10.4f}" for (name, val) in zip(sorted_keys, metrics_data)]
        )
...
def train(args):
    ...
    for i in range(args.start_run, args.total_run + 1):
        if args.restore_name is not None:
            args.restore_name_per_run = (
                "checkpoints/" + args.restore_name + f"-run-{i}.pth"
            )
        ...
        if best_sintel["clean-epe"] < 100:
            stats["sintel clean"] = stats.get("sintel clean", []) + [
                best_sintel["clean-epe"]
            ]
            stats["sintel final"] = stats.get("sintel final", []) + [
                best_sintel["final-epe"]
            ]
                    ...
                    if val_dataset == "chairs":
                        ...
                        results.update(
                            evaluate.validate_things(
                                model.module, sradius_mode=args.sradius_mode
                            )
                        )
    ...
    for val_dataset in args.validation:
        if val_dataset == "chairs":
            ...
            evaluate.validate_sintel(
                model.module,
                sradius_mode=args.sradius_mode,
                checkpoint_name=args.restore_ckpt,
            )
        elif val_dataset == "kitti":
        ...
        for split in args.viz_split:
            if viz_dataset == "sintel":
                ...
                viz.kitti_visualization(
                    model.module, split=split, output_path=args.output_path
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--fixed_point_reuse", action="store_true", help="Enable fixed point reuse."
    )
    ...
    parser.add_argument(
        "--start_run", type=int, default=1, help="begin from the given number of runs"
    )
    parser.add_argument("--restore_name", help="restore experiment name")
    parser.add_argument(
        "--resume_iter", type=int, default=-1, help="resume from the given iterations"
    )

    parser.add_argument(
        "--tiny", action="store_true", help="use a tiny model for ablation study"
    )
    ...
    parser.add_argument(
        "--old_version", action="store_true", help="use the old design for flow head"
    )
    ...
    parser.add_argument(
        "--eval_interval", type=int, default=5000, help="evaluation interval"
    )
    parser.add_argument(
        "--save_interval", type=int, default=5000, help="saving interval"
    )
    parser.add_argument(
        "--time_interval", type=int, default=500, help="timing interval"
    )
